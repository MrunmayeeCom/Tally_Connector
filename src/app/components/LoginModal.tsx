...
function LoginSuccessPopup({ name, onClose }: { name: string; onClose: () => void }) {
  // Stable ref so the timer never re-fires due to onClose reference changes
...
export function LoginModal({
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ── SIGN IN flow ──
      try {
        const userData = await loginCustomer(adminEmail, adminPassword);
        const displayName = userData.name || adminEmail.split("@")[0];

        localStorage.setItem("user", JSON.stringify({ name: displayName, email: adminEmail }));
        window.dispatchEvent(new Event("userLoginStatusChanged"));

        // Set popup BEFORE closing so both states land in the same render batch
  ...
  return (
    <>
      {/* AnimatePresence wraps the popup so exit animation plays when it unmounts */}
      <AnimatePresence>
        {showPopup && (
          <LoginSuccessPopup
            name={loggedInName}
