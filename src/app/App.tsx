...
import AgentUpdatePopup from "../AgentUpdatePopup"; // ← fixed path
import { checkAndNotify } from "../AgentUpdateService";         // ← adjust to wherever your file lives
...
export default function App() {
  ...
  useEffect(() => {
    checkAndNotify().then((url) => {
      if (url) setAgentFileUrl(url);
    });
  }, []);

  // Re-check when user logs in mid-session
  useEffect(() => {
    const onLogin = () => {
      checkAndNotify().then((url) => {
        if (url) setAgentFileUrl(url);
